...
class KeysWorker():
    ...
    def SendInput(self, *inputs):
        ...
        print(type(nInputs), type(pInputs), type(cbSize))
        print(nInputs, pInputs, cbSize)
        return ctypes.windll.user32.SendInput(nInputs, pInputs, cbSize)

    # send input
    def Send1Input(self, input):
        import pynput

        # extra = ctypes.c_ulong(0)
        # ii_ = pynput._util.win32.INPUT_union()
        # ii_.ki = pynput._util.win32.KEYBDINPUT(0, input, 0x0008, 0,
        ...
        LPINPUT = pynput._util.win32.INPUT_union()
        LPINPUT.ki = LPINPUT([input])
        pInputs = pynput._util.win32.INPUT(ctypes.c_ulong(1), LPINPUT)
        cbSize = ctypes.c_int(ctypes.sizeof(INPUT))
        print(type(pInputs), type(cbSize))
        return ctypes.windll.user32.SendInput(1, ctypes.pointer(pInputs), ctypes.sizeof(pInputs))
...
if __name__ == '__main__':
    ...
    for (x, y) in [(50, 50), (100, 50), (100, 100), (50, 100)]:
        keys.keys_worker.SendInput(keys.keys_worker.Mouse(keys.mouse_move, 50, 50))
        tm.sleep(0.5)
    print("sended ...")
